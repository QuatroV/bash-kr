...
lock_file="./lockfiles/zrdn1.lock"
...
function clear_messages(){
    rm -rf ./messages/zrdn1/*
}

ZRDN1_COORDINATE_X=3860000;
ZRDN1_COORDINATE_Y=4315000;
ZRDN1_RADIUS=600000;
...
do
    ...
    work_checks=`ls -t ./messages/kp/zrdn1 2>/dev/null`
    for work_check in $work_checks; do
        encrypted_info=`cat ./messages/kp/zrdn1/$work_check`
        decrypted_string=$(decipher_string "$encrypted_info" "$PASSPHRASE")
        rm ./messages/kp/zrdn1/$work_check

        msg="ЗРДН1 функционирует в нормальном режиме"
        encrypted_msg=$(cipher_string "$msg" "$PASSPHRASE")

        msg_name=$(generate_random_string)

        echo $encrypted_msg > ./messages/zrdn1/$msg_name.log
    done
    ...
    prev_targets=`cat ./temp/zrdn1_targets_first.log | cut -d, -f1`

    # Пробегаем по целям, обнаруженным на прошлом шаге
    prev_prev_targets=`cat ./temp/zrdn1_targets_second.log | cut -d, -f1`
    ...
    for target in $targets ; do
        ...
        target_in_area=`is_point_in_circle $ZRDN1_COORDINATE_X $ZRDN1_COORDINATE_Y $current_x $current_y $ZRDN1_RADIUS`
        ...
        if [ $ZRDN1_AMMO_AMOUNT -gt 0 ]; then
            has_ammo=true
        else
            has_ammo=false
            if [ $ZRDN1_AMMO_AMOUNT -eq 0 ]; then
                ((--ZRDN1_AMMO_AMOUNT))
        ...
        previous_previous_string=`grep $target_id ./temp/zrdn1_targets_second.log`
        ...
        previous_string=`grep $target_id ./temp/zrdn1_targets_first.log`

        # Если НЕ нашлась строка с таким ID на первой засечке
        if [ -z "$previous_string" ]; then
            # То записываем в конец файла ID цели
            echo "`echo $target | tail -c 7 `,`cat $DirectoryTargets/$target`" >> ./temp/zrdn1_targets_first.log
        else
            ...
            if [ $target_type == "cruise_missle" ] && $has_ammo; then
                ...
                echo "`echo $target | tail -c 7 `,`cat $DirectoryTargets/$target`" >> ./temp/zrdn1_targets_second.log
