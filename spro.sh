...
do
    ...
    work_checks=`ls -t ./messages/kp/spro 2>/dev/null`
    for work_check in $work_checks; do
        encrypted_info=`cat ./messages/kp/spro/$work_check`
        decrypted_string=$(decipher_string "$encrypted_info" "$PASSPHRASE")
        rm ./messages/kp/spro/$work_check

        msg="СПРО функционирует в нормальном режиме"
        encrypted_msg=$(cipher_string "$msg" "$PASSPHRASE")

        msg_name=$(generate_random_string)

        echo $encrypted_msg > ./messages/spro/$msg_name.log
    done
    ...
    prev_targets=`cat ./temp/spro_targets_first.log | cut -d, -f1`

    # Пробегаем по целям, обнаруженным на прошлом шаге
    prev_prev_targets=`cat ./temp/spro_targets_second.log | cut -d, -f1`
    ...
    for target in $targets ; do
        ...
        target_in_area=`is_point_in_circle $SPRO_COORDINATE_X $SPRO_COORDINATE_Y $current_x $current_y $SPRO_RADIUS`
        ...
        if [ $SPRO_AMMO_AMOUNT -gt 0 ]; then
            has_ammo=true
        else
            has_ammo=false
            if [ $SPRO_AMMO_AMOUNT -eq 0 ]; then
                ((--SPRO_AMMO_AMOUNT))
        ...
        previous_previous_string=`grep $target_id ./temp/spro_targets_second.log`
        ...
        previous_string=`grep $target_id ./temp/spro_targets_first.log`

        # Если НЕ нашлась строка с таким ID на первой засечке
        if [ -z "$previous_string" ]; then
            # То записываем в конец файла ID цели
            echo "`echo $target | tail -c 7 `,`cat $DirectoryTargets/$target`" >> ./temp/spro_targets_first.log
        else
            ...
            if [ $target_type == 'ballistic_missle' ] && $has_ammo; then
                msg="Обнаружена баллистическая ракета на координатах x:$current_x, y:$current_y. Производится выстрел."
                ...
                echo "`echo $target | tail -c 7 `,`cat $DirectoryTargets/$target`" >> ./temp/spro_targets_second.log
