...
function clear_logs(){
    ...
    echo "" > ./temp/rls2_targets_first.log
}
...
RLS2_COORDINATE_X=9500000
RLS2_COORDINATE_Y=3000000
RLS2_RADIUS=3000000
RLS2_ANGLE_START=80
RLS2_ANGLE_END=190
...
do
    ...
    work_checks=`ls -t ./messages/kp/rls2 2>/dev/null`
    for work_check in $work_checks; do
        encrypted_info=`cat ./messages/kp/rls2/$work_check`
        decrypted_string=$(decipher_string "$encrypted_info" "$PASSPHRASE")
        rm ./messages/kp/rls2/$work_check

        msg="РЛС2 функционирует в нормальном режиме"
        encrypted_msg=$(cipher_string "$msg" "$PASSPHRASE")

        msg_name=$(generate_random_string)

        echo $encrypted_msg > ./messages/rls2/$msg_name.log
    done
    ...
    for target in $targets ; do
        ...
        # Arguments: center_x, center_y, radius, angle1, angle2, point_x, point_y
        target_in_area=`is_point_in_sector $RLS2_COORDINATE_X $RLS2_COORDINATE_Y $RLS2_RADIUS $RLS2_ANGLE_START $RLS2_ANGLE_END $current_x $current_y `
        ...
        previous_string=`grep $target_id ./temp/rls2_targets_first.log`

        if [ -z "$previous_string" ]; then
            # То записываем в конец файла ID цели
            echo "`echo $target | tail -c 7 `,`cat $DirectoryTargets/$target`" >> ./temp/rls2_targets_first.log
            ...
            previous_string_line_number=`grep -n $target_id ./temp/rls2_targets_first.log`
            new_line="`echo $target | tail -c 7 `,`cat $DirectoryTargets/$target`"
            sed -i "s/$previous_string/$new_line/g" "./temp/rls2_targets_first.log"
