...
lock_file="./lockfiles/zrdn3.lock"
...
function clear_messages(){
    rm -rf ./messages/zrdn3/*
}

ZRDN3_COORDINATE_X=3225000;
ZRDN3_COORDINATE_Y=2680000;
ZRDN3_RADIUS=550000;
...
do
    ...
    work_checks=`ls -t ./messages/kp/zrdn3 2>/dev/null`
    for work_check in $work_checks; do
        encrypted_info=`cat ./messages/kp/zrdn3/$work_check`
        decrypted_string=$(decipher_string "$encrypted_info" "$PASSPHRASE")
        rm ./messages/kp/zrdn3/$work_check

        msg="ЗРДН3 функционирует в нормальном режиме"
        encrypted_msg=$(cipher_string "$msg" "$PASSPHRASE")

        msg_name=$(generate_random_string)

        echo $encrypted_msg > ./messages/zrdn3/$msg_name.log
    done
    ...
    prev_targets=`cat ./temp/zrdn3_targets_first.log | cut -d, -f1`

    # Пробегаем по целям, обнаруженным на прошлом шаге
    prev_prev_targets=`cat ./temp/zrdn3_targets_second.log | cut -d, -f1`
    ...
    for target in $targets ; do
        ...
        target_in_area=`is_point_in_circle $ZRDN3_COORDINATE_X $ZRDN3_COORDINATE_Y $current_x $current_y $ZRDN3_RADIUS`
        ...
        if [ $ZRDN3_AMMO_AMOUNT -gt 0 ]; then
            has_ammo=true
        else
            has_ammo=false
            if [ $ZRDN3_AMMO_AMOUNT -eq 0 ]; then
                ((--ZRDN3_AMMO_AMOUNT))
        ...
        previous_previous_string=`grep $target_id ./temp/zrdn3_targets_second.log`
        ...
        previous_string=`grep $target_id ./temp/zrdn3_targets_first.log`

        # Если НЕ нашлась строка с таким ID на первой засечке
        if [ -z "$previous_string" ]; then
            # То записываем в конец файла ID цели
            echo "`echo $target | tail -c 7 `,`cat $DirectoryTargets/$target`" >> ./temp/zrdn3_targets_first.log
        else
            ...
            if [ $target_type == "cruise_missle" ] && $has_ammo; then
                ...
                echo "`echo $target | tail -c 7 `,`cat $DirectoryTargets/$target`" >> ./temp/zrdn3_targets_second.log
