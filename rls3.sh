...
function clear_logs(){
    ...
    echo "" > ./temp/rls3_targets_first.log
}
...
RLS3_COORDINATE_X=3815000
RLS3_COORDINATE_Y=3905000
RLS3_RADIUS=3000000
RLS3_ANGLE_START=120
RLS3_ANGLE_END=240
...
do
    ...
    work_checks=`ls -t ./messages/kp/rls3 2>/dev/null`
    for work_check in $work_checks; do
        encrypted_info=`cat ./messages/kp/rls3/$work_check`
        decrypted_string=$(decipher_string "$encrypted_info" "$PASSPHRASE")
        rm ./messages/kp/rls3/$work_check

        msg="РЛС3 функционирует в нормальном режиме"
        encrypted_msg=$(cipher_string "$msg" "$PASSPHRASE")

        msg_name=$(generate_random_string)

        echo $encrypted_msg > ./messages/rls3/$msg_name.log
    done
    ...
    for target in $targets ; do
        ...
        # Arguments: center_x, center_y, radius, angle1, angle2, point_x, point_y
        target_in_area=`is_point_in_sector $RLS3_COORDINATE_X $RLS3_COORDINATE_Y $RLS3_RADIUS $RLS3_ANGLE_START $RLS3_ANGLE_END $current_x $current_y `
        ...
        previous_string=`grep $target_id ./temp/rls3_targets_first.log`

        if [ -z "$previous_string" ]; then
            # То записываем в конец файла ID цели
            echo "`echo $target | tail -c 7 `,`cat $DirectoryTargets/$target`" >> ./temp/rls3_targets_first.log
            ...
            previous_string_line_number=`grep -n $target_id ./temp/rls3_targets_first.log`
            new_line="`echo $target | tail -c 7 `,`cat $DirectoryTargets/$target`"
            sed -i "s/$previous_string/$new_line/g" "./temp/rls3_targets_first.log"
